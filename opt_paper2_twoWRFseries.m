...
if length(elev_profile_m{i}) > 1
    ...
    function [f_out, x_out_raw, x_out_pp, energyCostNominal_out, ET_actual_mgd] = opt_subroutine
        % opt_subroutine(i,j,isPipe,numWRFs,elev_profile_ft,elev_profile_m, rwCapacity_mgd, po, z0, USD2011to2015,lccFactor_constr_convey_pump,lccFactor_om_convey, a,b,c,d, om_fixed_treat, assessmentPeriod_yr, lccFactor_constr_convey_pipe)
        ...
        for m = 1:numWRFs
            if m == 1 % for the terminal plant, the capacity of conveyance to spreading basin is equal to the sum of the individual production capacities
                [pipeLength_ft(m), pipeFeasible_index{m}, pump_reqs{m}, prv_reqs{m}] = pipeline_routine(elev_profile_m{m}, rwCapacity_afy(1) + rwCapacity_afy(2), po);
            else
        ...
        %% Special heuristic: do not run for the 13th pipe size because there seem to be issues with it over 30 year planning period
        if pipeFeasible_indexNum{1}(end) == 13
            pipeFeasible_indexNum{1}(end) = [];
        end
        
        fstarMatrix = zeros(length(pipeFeasible_indexNum{1}),length(pipeFeasible_indexNum{2}));
        xstarMatrix = zeros(length(pipeFeasible_indexNum{1}),length(pipeFeasible_indexNum{2}),numberDecisionVariables);
        energyCostNominal_array = fstarMatrix; % initialize matrix with same structure as fstarMatrix to hold energy cost
        
        nonlcon = [];
        
        for k = 1:length(pipeFeasible_indexNum{1})
            ...
            for m = 1:length(pipeFeasible_indexNum{2}) % iterate over all the feasible pipe sizes
                % Compute the fixed costs for pipeline and pump stations
                for n = 1:numWRFs
                    if n == 1
                ...
                while abs(fstar)>opt_precision
                    ...
                    if iter > 25 && abs(fstar) < opt_precision*10
                        problemCaps{i,j} = [problemCaps{i,j}; pipeFeasible_indexNum{1}(k), pipeFeasible_indexNum{2}(m), fstar];
                        break
                    end
                    ...
                    if iter > 35 && abs(fstar) > 0.5
                        problemCaps{i,j} = [problemCaps{i,j}; pipeFeasible_indexNum{1}(k), pipeFeasible_indexNum{2}(m), fstar];
                        disp('Quitting optimizaton: could not converge to solution. Check problemCaps for more information')
                        break
                    end
                    
                    if iter > 40
                        problemCaps{i,j} = [problemCaps{i,j}; pipeFeasible_indexNum{1}(k), pipeFeasible_indexNum{2}(m), fstar];
                        disp('Quitting optimizaton: could not converge to solution after max number of iterations. Check problemCaps for more information')
                        break
                    end
                                        
                    f_df = @(z) funit_lcc_daily_nonlinearCost_twoWRFserial(z, rwCapacity_mgd,a,b,c,d,onlineFactor,om_fixed_treat,assessmentPeriod_yr,conveyCost_fixed,H_ft,hfs_vector,energyCost_perMGDperftperday,alpha_param, rw_indices, infiltration_indices, discountFactor_average);
                    
                    hessianfcn = @(z,lambda) hessianfcn_dailytwoWRFserial(z,lambda,rwCapacity_mgd,a,b,c,d,onlineFactor,om_fixed_treat,assessmentPeriod_yr,conveyCost_fixed,H_ft,hfs_vector,energyCost_perMGDperftperday, rw_indices, infiltration_indices, discountFactor_average);
                    
                    options = optimoptions('fmincon','Algorithm','interior-point', 'GradObj','on','HessFcn',hessianfcn,'Display','notify-detailed','OptimalityTolerance',opt_precision);
                ...
                alpha3 = [sum((xstar(rw_indices{1}) + xstar(rw_indices{2})).^2.85), sum(xstar(rw_indices{2}).^2.85)];
                
                energyCost_convey_total = dot(alpha3, alpha2) + energyCost_perMGDperftperday * (H_ft(1) * sum(xstar(rw_indices{1})+ xstar(rw_indices{2})) + H_ft(2) * sum(xstar(rw_indices{2}))); % compute the total lifetime energy cost. Note that the most downstream WRF, indexed as WRF 1 will have to pump the full load.
